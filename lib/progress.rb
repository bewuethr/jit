class Progress
  ...
  def start(message, total = nil)
    return if ENV["NO_PROGRESS"] || !@output.isatty
  ...
  private def get_time = Process.clock_gettime(Process::CLOCK_MONOTONIC)

  private def clear_line = @output.write("\e[G\e[K")

  private def status_line
