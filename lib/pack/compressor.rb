...
module Pack
  class Compressor
    ...
    private def build_delta(entry)
      object = @database.load_raw(entry.oid)
      target = @window.add(entry, object.data)

      @window.each { try_delta(_1, target) }
    end

    private def try_delta(source, target)
    ...
    private def max_size_heuristic(source, target)
      if target.delta
    ...
    private def compatible_sizes?(source, target, max_size)
