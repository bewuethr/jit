...
module Pack
  class Stream
    ...
    def readbyte = read(1).bytes.first
    ...
    private def new_byte_string = String.new("", encoding: Encoding::ASCII_8BIT)

    private def update_state(data)
      @digest.update(data) unless @capture
      @offset += data.bytesize
      @capture&.concat(data)
    end

    private def read_buffered(size, block = true)
