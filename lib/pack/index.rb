module Pack
  class Index
    ...
    private def read_int32(layer, pos)
      @input.seek(offset_for(layer, pos))
      @input.read(4).unpack1("N")
    end

    private def binary_search(target, low, high)
      while low <= high
        mid = (low + high) / 2
