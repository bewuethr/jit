...
module Command
  class ReceivePack < Base
    ...
    private def recv_update_requests
      @requests = {}

      @conn.recv_until(nil) do |line|
        old_oid, new_oid, ref = line.split(/ +/)
        @requests[ref] = [old_oid, new_oid].map { zero_to_nil(_1) }
      end
    end

    private def zero_to_nil(oid) = (oid == ZERO_OID) ? nil : oid
