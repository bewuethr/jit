...
module Command
  class Checkout < Base
    ...
    private def print_previous_head
      if @current_ref.head? && @current_oid != @target_oid
        print_head_position("Previous HEAD position was", @current_oid)
      end
    end

    private def print_detachment_notice
      return unless @new_ref.head? && !@current_ref.head?
    ...
    private def print_head_position(message, oid)
