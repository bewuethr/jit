...
module Command
  class Revert < Base
    ...
    private def revert_merge_inputs(commit)
      ...
      ::Merge::CherryPick.new(left_name, right_name,
        left_oid, right_oid, [commit.oid])
    end

    private def revert_commit_message(commit)
      <<~EOF
    ...
    private def edit_revert_message(message)
